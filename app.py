...
with st.sidebar:
    ...
    with st.form("user_info_form"):
        birth_date = st.date_input("阳历生日", value=None, min_value=date(1960, 1, 1), max_value=date.today())
        ...
        birth_time_idx = st.selectbox("出生时辰", range(len(time_options)), format_func=lambda x: time_options[x])
        gender = st.selectbox("性别", ["男", "女"])
        birth_place = st.text_input("出生地", placeholder="必填，如: 北京市海淀区", value="")
        ...
        user_mbti = st.selectbox("你的MBTI性格类型 (选填)", mbti_options)
